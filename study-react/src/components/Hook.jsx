...
const Hook = () => {
    ...
    const changeCount = () => {
        ...
        // 실행한 결과: 2
        // 아래 작성한 코드는 비동기로진행이되어서 원하는 값이 나오지 않는다
        setCount(count+1);
        setCount(count+1);
        setCount(count+1);
    }
    ...
    return (  
        <div>
            ...
            <h3>useState</h3>
            <p>
                컴포넌트 안에서 state를 사용할수 있게하는 hook
                const [사용할 변수이름, 값을 지정할 함수] = useState(들어갈 값) <br />
                여러개 작성 가능, 들어갈 값에는 모든 자료형이 들어갈수 있다<br />
                !useState는 비동기 값을 넣어줌! : 값을 복잡하게 연속적 넣어줄때 문제가 생길수도 있다.
