...
const NumberBox = () => {
    ...
    /** 값을 가져올때 그 값을 확인해주지 않기 때문에 이름 확인
    ...
    const text = useSelector((state)=>(state.number.text));

    // 리덕스에서 작성한 리듀서를 가져오기위한 useDispatch
    // dispatch를 사용해줄때, 
    // dispatch({type:값}) 전달 -type통해서 리듀서를 찾아감
    ...
    return (  
        <div>
            ...
            <p>useSelector를 통해서 가져온 값 : {text}</p>
            <button onClick={()=>{dispatch({type:"increase"})}}>(type으로)1증가</button>
            {/* 액션함수를 사용할때 그 함수의 결과값이 들어가게 작성 */}
            <button onClick={()=>{dispatch(increase())}}>(함수로)1증가</button>
